...
BME680::BME680(PinName sda, PinName scl) : BME680(sda, scl, BME680_DEFAULT_ADDRESS) {
}

BME680::BME680(PinName sda, PinName scl, uint8_t adr) : i2c(sda, scl) {
...
bool BME680::begin() {
    ...
    /*
     * Support single instance only
     *
     * To support multiple instance, we need std::function to store lambda
     * expression (C++ closure) or bind expression (std::bind). However,
     * Mbed OS doesn't enable C++ RTTI, causing std::function target()
     * always returns nullptr for typeid() unsupported.
     *
     * NOTE: We can cast capture-less lambda to function pointer, but not
     *       for capturing lambda. This needs std::function anyway.
     */
    static BME680 *StaticThis = this;
    gas_sensor.read = [](uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) -> int8_t {
        return StaticThis->i2c_read(dev_id, reg_addr, reg_data, len);
    };
    gas_sensor.write = [](uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) -> int8_t {
        return StaticThis->i2c_write(dev_id, reg_addr, reg_data, len);
    };
    gas_sensor.delay_ms = [](uint32_t ms) -> void {
        return StaticThis->delay_msec(ms);
    };
    MBED_ASSERT(gas_sensor.read);
    MBED_ASSERT(gas_sensor.write);
    MBED_ASSERT(gas_sensor.delay_ms);
